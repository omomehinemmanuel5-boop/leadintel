import { geminiModel } from "@/lib/providers/geminiConfig";
import { Company, Contact } from "@/lib/types";

/**
 * Thin wrapper around Gemini's generateContent endpoint. Every prompt in
 * this file explicitly instructs the model to only use the facts it's
 * given and to say so when something isn't known — this pipeline's data
 * is already partly demo/partly real (see Companies/Contacts pages), and
 * an AI layer that confidently fabricates on top of that would make the
 * whole tool less trustworthy, not more useful.
 */
...
async function callGemini(prompt: string): Promise<string | null> {
  ...
  try {
    const res = await fetch(`${API_BASE}/${geminiModel()}:generateContent?key=${apiKey}`, {
      method: "POST",
...
export async function generateCompanySummary(company: Company, contact: Contact | null): Promise<string | null> {
  const facts = [
    ...
    contact ? `Known leader: ${contact.name}, ${contact.title} (source: ${contact.discoverySource})` : `No leader identified yet`,
  ].join("\n");
